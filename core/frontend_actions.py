...
def edit_offer_view(request, offer_id):
    opp = get_object_or_404(Opportunity, id=offer_id)
    user = request.user
    ...
    if request.method == 'POST':
        ...
        opp.vacancies = int(request.POST.get('required_hours') or opp.vacancies)
...
# Support/chat persistence endpoint for chatbot
...
def save_chat_message(request):
    ...
    try:
        ...
        # If saving fails, ignore so the chatbot still works client-side
